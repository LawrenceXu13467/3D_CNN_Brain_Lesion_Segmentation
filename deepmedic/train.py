...
def is_valid_pass(devArg) :
    if devArg == ARG_CPU_PROC: return
    if devArg == ARG_GPU_PROC: return
    if devArg.startswith(ARG_GPU_PROC) and check_is_int(devArg[ len(ARG_GPU_PROC) : ]) : return
...
def parserSettings() :
    ...
    parser.add_argument(NEW_MOD, dest='model_conf', type=str, help="Create a new CNN model with model parameters at given config file [MODEL_CONF].")
    parser.add_argument(OPT_TRAIN, dest='training_conf', type=str, help="Train a CNN model with training parameters at given config file [TRAINING_CONF].\n"+\
                                                                    "This option can follow a ["+NEW_MOD+" MODEL_CONF] option, to create a new model and start training it immediately.\n"+\
    ...
    parser.add_argument(OPT_TEST, dest='testing_conf', type=str, help="Test with an existing CNN model. The testing session's parameters should be given in config file [TESTING_CONF].\n"+\
                                                                    ...
                                                                    "This option cannot be used in combination with ["+NEW_MOD+"] or ["+OPT_TRAIN+"].")
    parser.add_argument(OPT_MODEL, dest='saved_model', type=str, help="The path to a saved existing cnn model, to train or test with.\n"+\
                                                                    "This option can follow a ["+OPT_TRAIN+"] or ["+OPT_TEST+"] option. Not in combination with a ["+NEW_MOD+"].\n"+\
    ...
    parser.add_argument(OPT_PRETR, dest='pretrained_model', type=str, help="Use to transfer the weights from a previously trained model to a new model.\n"+\
                                                                    "This option must follow a ["+NEW_MOD+"] option.\n"+\
                                                                    "Usage: ./deepMedicRun "+NEW_MOD+" /path/to/model/config "+OPT_PRETR+" /path/to/pretrained/model \n"+\
    ...
    parser.add_argument(OPT_PRETR_LAYERS, dest='layers_to_transfer', nargs='+', type=int, help="Use only after a ["+OPT_PRETR+"] option.\n"+\
                                                                    ...
                                                                    "Usage: ./deepMedicRun "+NEW_MOD+" /path/to/model/config "+OPT_PRETR+" /path/to/pretrained/model "+OPT_PRETR_LAYERS+" 1 2 3 ...")
...
if __name__ == '__main__':
    ...
    if args.model_conf and args.saved_model :
        print("ERROR:\t["+NEW_MOD+"] and ["+OPT_MODEL+"] options cannot be used together.\n"+\
    ...
    if args.testing_conf and (args.model_conf or args.training_conf) :
        print("ERROR:\t["+OPT_TEST+"] cannot be used in conjuction with ["+NEW_MOD+"] or ["+OPT_TRAIN+"].\n"+\
    ...
    if args.pretrained_model and not args.model_conf :
        print("ERROR:\tThe option ["+OPT_PRETR+"] can only be used together with the ["+NEW_MOD+"] option.\n\tPlease try -h for more information. Exiting."); exit(1)
